...
# Title of the app
st.set_page_config(page_title="Produtividade CX - MOVI", page_icon=":bar_chart:", layout="wide")
...
RankingSemana = RankingSemana.drop(columns=['Ticket','Ação nº'])
...
# criando a variavel velocidade
	...
	st.subheader('MoviDesk - 01/09/2021 a 28/09/2021')
	st.markdown('#')
...
with columnS:
	...
	plot1 = go.Figure(data=[go.Bar(name= 'TMA',x=Analise_Desempenho.index,y=Analise_Desempenho['TMA(min)']),go.Line(name="Média TMA da Equipe",x=Analise_Desempenho.index,y=Analise_Desempenho['Média TMA Equipe']),go.Line(name='Meta Velocidade',x=Analise_Desempenho.index,y=Analise_Desempenho['Meta TMA'])])
...
with left_column:
	...
	Mapeando_Status_Bonito = Mapeando_Status.drop(columns=['Ticket','Ação nº','Minutos Trabalhados'])
	Ranking_Status = Mapeando_Status_Bonito.sort_values('Atendimentos',ascending=False)
...
plot = go.Figure(data=[go.Bar(name= 'Atendimentos',x=Operador_Atendimentos_Data.index,y=Operador_Atendimentos_Data['Atendimentos']),go.Line(name="Média Atendimentos Individual",x=Operador_Atendimentos_Data.index,y=Operador_Atendimentos_Data['Média Atendimentos Equipe']), go.Line(name="Meta Atendimentos Individual",x=Operador_Atendimentos_Data.index,y=Operador_Atendimentos_Data['Meta Atendimentos']), go.Line(name="Linha de Tendência",x=Operador_Atendimentos_Data.index,y=Operador_Atendimentos_Data['Atendimentos']),go.Line(name="Potencial da Equipe",x=Operador_Atendimentos_Data.index,y=Operador_Atendimentos_Data['Meta Atendimentos Equipe'])])
...
st.markdown('Teria a opção de exportar documentos individuais para cada operador, bem como para a operação em geral, visto que basta selecionar todos os Agentes no filtro ao lado')
# atribuindo uma nova coluna NOTA
