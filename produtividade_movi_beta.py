...
input_Dias_Analisados = st.number_input('Quantos dias úteis vou analisar?',min_value=1,max_value=30,value=21,step=1)
...
input_Atendimentos_Meta = st.number_input('Digite a meta de atendimentos diários da equipe',min_value=1,max_value=500,value=125,step=1)
...
if input_Dias_Analisados is not None:
	...
	with column5:
		...
		def get_table_download_link(dataframe):
		    ...
		    val = to_excel(dataframe)
		    b64 = base64.b64encode(val)  # val looks like b'...'
		    return f'<a href="data:application/octet-stream;base64,{b64.decode()}" download="extract.xlsx">Ranking Parceiros!</a>' # decode b'abc' => abc

		dataframe = Ranking_Parceiros # your dataframe
		st.markdown(get_table_download_link(dataframe), unsafe_allow_html=True)
